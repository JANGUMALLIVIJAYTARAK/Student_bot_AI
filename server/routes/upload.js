...
router.post('/', tempAuth, (req, res) => {
    ...
    uploader(req, res, async function (err) {
        ...
        // Pass the UNIQUE SERVER FILENAME to the Python service as the identifier.
        triggerPythonRagProcessing(userId, absoluteFilePath, serverFilename) // <-- CRITICAL CHANGE
            .then(ragResult => {
                if (!ragResult.success) {
                     console.error(`Background RAG processing failed for ${serverFilename} (User: ${userId}): ${ragResult.message}`);
                } else {
                     console.log(`Background RAG processing initiated/completed for ${serverFilename} (User: ${userId}). Status: ${ragResult.status}`);
                }
            })
            .catch(error => {
                 console.error(`Error initiating background Python RAG processing for ${serverFilename}:`, error);
            });
